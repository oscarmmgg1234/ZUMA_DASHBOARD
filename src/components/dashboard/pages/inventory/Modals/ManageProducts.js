...
export default function ManageProducts(props) {
  ...
  const tableRows = filteredProducts.map((product, index) => (
    ...
    const handleAddProduct = () => {
      if (!productName || !description || !price || !type || !location || !company || !unitBundle) {
    ...
    <div className="h-full w-full">
        ...
        {action ? (
                    <div className="flex flex-col space-y-3">
                        ...
                        <input type="text" placeholder="Product Name" value={productName} onChange={(e) => setProductName(e.target.value)} className="h-10 px-3 rounded border border-gray-300 text-black"/>

                        <label className="font-semibold text-gray-700">Description</label>
                        <input type="text" placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} className="h-10 px-3 rounded border border-gray-300  text-black"/>

                        <label className="font-semibold text-gray-700">Price</label>
                        <input type="text" placeholder="Price" value={price} onChange={(e) => setPrice(e.target.value)} className="h-10 px-3 rounded border border-gray-300  text-black"/>

                        <label className="font-semibold text-gray-700">Type</label>
                        <input type="text" placeholder="Type" value={type} onChange={(e)=> setType(e.target.value)}  className="h-10 px-3 rounded border border-gray-300  text-black" /> 
                       
                        <label className="font-semibold text-gray-700">Location</label>
                        <input type="text" placeholder="Type" value={location} onChange={(e)=> setLocation(e.target.value)}  className="h-10 px-3 rounded border border-gray-300  text-black" /> 

                        <label className="font-semibold text-gray-700">Company</label>
                        <input type="text" placeholder="Type" value={company} onChange={(e)=> setCompany(e.target.value)}  className="h-10 px-3 rounded border border-gray-300  text-black" /> 

                        <label className="font-semibold text-gray-700">Process Component Type</label>
                        <input type="text" placeholder="Process Component Type" value={processComponentType} onChange={(e) => setProcessComponentType(e.target.value)} className="h-10 px-3 rounded border border-gray-300  text-black"/>

                        <label className="font-semibold text-gray-700">Process Type</label>
                        <input type="text" placeholder="Process Type" value={processType} onChange={(e) => setProcessType(e.target.value)} className="h-10 px-3 rounded border border-gray-300  text-black"/>

                        <label className="font-semibold text-gray-700">Reduction Type</label>
                        <input type="text" placeholder="Reduction Type" value={reductionType} onChange={(e) => setReductionType(e.target.value)} className="h-10 px-3 rounded border border-gray-300  text-black"/>
                        ...
                        <button onClick={()=>handleAddProduct()} className="w-full h-12 bg-blue-500 hover:bg-blue-600 text-white rounded-md">Add Product</button>
                    </div>
