...
import { transform_shift } from "../utils/transform_shift";
import { removeShift } from "../utils/removeShift";
...
const processShiftChanges = async (
  ...
  for (const date in shiftData) {
    ...
    if (current.calledOff !== original.calledOff) {
      changes.push(() =>
        removeShift({
          e_id: employeeId,
          date,
          revert: current.calledOff, // true = restore, false = remove
        })
      );
    }
  }
