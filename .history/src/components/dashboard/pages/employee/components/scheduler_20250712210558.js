...
const MyCalendar = ({ employee }) => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      const data = await https.getShiftLogs(employee.id, "2025-06-29", "2025-8-02");
   const formattedData = {};

   data.forEach((item) => {
     const localDate = new Date(item.SHIFT_DATE);
     const key = localDate.toISOString().split("T")[0]; // 'YYYY-MM-DD'

     formattedData[key] = {
       SHIFT_START: item.SHIFT_START?.slice(11, 16), // extract 'HH:MM'
       SHIFT_END: item.SHIFT_END?.slice(11, 16),
       VALID: !!item.VALID,
     };
   });
