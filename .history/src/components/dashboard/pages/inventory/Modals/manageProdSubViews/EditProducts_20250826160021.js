...
const runtimeTestHandler = async (productID) => {
  // open the tab synchronously so it won't be blocked
  const win = window.open("", "<h1>Generating Test Report</h1>");
  if (!win) {
  ...
  try {
    const html = await http.runtimeTest({ productID }); // returns full <!doctype html> string
    ...
    // clean up later (kept alive for 2 minutes)
