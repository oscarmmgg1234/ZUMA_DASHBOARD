...
function ActionNode({ data, id }) {
  ...
  const [params, setParams] = useState({});         // { main_1: "...", opt_0: "...", ... }
  ...
  // Push structured params + legacy mirrors
  const pushParams = useCallback(() => {
    pushField("params", params);
    const ordered = Object.keys(params).sort(); // main_1, main_2, opt_0, opt_1...
  ...
  // hydrate from upstream token (only when identity changes)
  useEffect(() => {
    if (!data.token) return;
    setSelectedClass((data.token.type || "").toUpperCase());
    setSelectedFunctionId(data.token.func || "");

    // migrate legacy -> structured (remember main_0 is productId, hidden)
    const next = {};
    if (data.token.param1) next["main_1"] = data.token.param1;
  ...
  // auto-pick function if only one available for class
  useEffect(() => {
  ...
  const visibleMain = Math.max(0, rawMain - 1);  // exclude main_0 (productId)
  const optCount = Math.max(0, meta.optionalParams || 0);

  // descs handling: some functions include productId in optionalDesc[0], some don't.
  const descs = Array.isArray(meta.optionalDesc) ? meta.optionalDesc : [];
  const includesProductIdDesc = descs.length === (rawMain + optCount); // full list includes prodId
  const getDesc = (idx, fallback) => (descs[idx] && descs[idx].desc) || fallback;

  // build labels in UI order: required mains (excluding prodId), then optionals
  const labels = [];
  // required mains
  ...
  for (let j = 0; j < optCount; j++) {
    const base = includesProductIdDesc ? rawMain : 0; // if list includes main labels, jump over them
    labels.push(getDesc(base + j, `Optional param ${j + 1}`));
  }

  // turn labels into field descriptors
  const fields = [];
  for (let i = 0; i < visibleMain; i++) {
    fields.push({ key: `main_${i + 1}`, label: labels[i], required: true });
  }
  for (let j = 0; j < optCount; j++) {
  ...
  return (
    ...
    >
      ...
      {/* Required fields (excluding productId) */}
