...
export default function CustomProduct(props) {
  ...
  // NEW: success banner state (persists after clearing the form)
  const [lastCreated, setLastCreated] = useState(null);
  // shape: { productID, name, linked, poolName }
  ...
  const fieldsValid =
    requiredFields.every(
      (f) => (formFields[f] ?? "").toString().trim() !== ""
    ) &&
    !Number.isNaN(Number(formFields.price)) &&
    Number(formFields.price) >= 0;
  const poolReady = !usePool || !!linkStatus?.linked;
  const canCreate = fieldsValid && poolReady;
  ...
  const handleCreateProduct = async () => {
    if (!canCreate) {
      if (!fieldsValid)
        return toast("Fill out all required fields (and valid price).");
      if (!poolReady)
        return toast(
          "Link the product to a pool first, or disable pool usage."
        );
    }
    ...
    try {
      ...
      const res = await http.addProductProcess(packet);
      const ok =
        res?.success === true ||
        res?.status === true ||
        res?.created === true ||
        !!res?.createdProduct ||
        // if API doesn't return a flag, treat the absence of error as OK
        typeof res === "object";

      if (!ok) {
        throw new Error(res?.message || res?.status || "Create failed");
      }

      // success banner shows the created info (even after we clear the form)
      setLastCreated({
        productID,
        name: formFields.name.trim(),
        linked: !!linkStatus?.linked,
        poolName: linkStatus?.poolName || null,
      });
      ...
      setIsLoading(false);
      if (phase === "creating") setPhase("form");
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      {/* NEW: success banner (dismissible) */}
      {lastCreated && (
        <div style={styles.bannerSuccess} role="status" aria-live="polite">
          <div
            style={{
              display: "flex",
              gap: 8,
          ...
          >
            ...
            <span>
              ...
              {lastCreated.name ? <>{lastCreated.name} • </> : null}
              <span style={{ fontFamily: "monospace" }}>
                {lastCreated.productID}
              </span>
              {lastCreated.linked && lastCreated.poolName ? (
                <>
                  {" "}
                  • Linked to pool <strong>{lastCreated.poolName}</strong>
                </>
              ) : null}
            </span>
          </div>
      ...
      <form style={styles.form} onSubmit={(e) => e.preventDefault()}>
        ...
        >
          <div style={styles.row}>
            ...
            <div style={styles.field}>
              ...
              <select
                ...
                onChange={propChange}
                style={styles.input(
                  (formFields.company ?? "").toString().trim() !== ""
                )}
              >
            ...
            <div style={styles.field}>
              ...
              <select
                ...
                onChange={propChange}
                style={styles.input(
                  (formFields.unitType ?? "").toString().trim() !== ""
                )}
              >
          ...
          <button
            ...
            style={canCreate ? styles.submit : styles.submitDisabled}
            disabled={!canCreate}
          >
            Create Product
          </button>
