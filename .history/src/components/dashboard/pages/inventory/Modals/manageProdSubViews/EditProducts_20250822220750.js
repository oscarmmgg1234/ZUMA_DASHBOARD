...
const deleteProductHandler = async (productID) => {//so the product id is the selected product but first i think we need some kind of warning system then ionce u select yes for are u sure then it goes back to select proiduct button and updates the proiduct list 
  await http.manageProducts({ productID, action: "delete" });
};
