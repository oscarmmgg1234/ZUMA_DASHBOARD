...
const parseTokensFromProduct = async (product) => {
  ...
  const [actRes, redRes, shipRes] = await Promise.all([
    http._tokenPreCheck({token: product.ACTIVATION_TOKEN, productproduct.PRODUCT_ID),
    http._tokenPreCheck(product.REDUCTION_TOKEN, product.PRODUCT_ID),
    http._tokenPreCheck(product.SHIPMENT_TOKEN, product.PRODUCT_ID),
  ]);
