...
function FlowComponentInner() {
  ...
  const onKeyDown = useCallback((e) => {
    if (e.key === 'Backspace' || e.key === 'Delete') {
      ...
      setTree(prev => {
        let newTree = prev.filter((node, idx, arr) => {
          if (node.id === id) {
            // Reconnect neighboring products
  ...
  return (
    <div style={{ width: '100vw', height: '100vh' }} tabIndex={0} onKeyDown={onKeyDown}>
      <ReactFlow
        nodes={rfNodes}
        edges={rfEdges}
