...
function buildLayout(tree) {
  ...
  const tokenToNodeMap = (token) => {
    
  }

const parseTokenString = (tokenString, productId) => {
  if (!tokenString) return [];

  return tokenString.split(' ').map((entry) => {
    const [type, func, id, p1, p2, p3] = entry.split(':');
    return {
      type: type?.toLowerCase() || null,
      func: func?.toLowerCase() || null,
      productId: id?.toLowerCase() || productId?.toLowerCase() || null,
      param1: p1 || null,
      param2: p2 || null,
      param3: p3 || null,
    };
...
const buildTokenMap = (products) => {
  ...
  products.forEach(product => {
    ...
    const activations = parseTokenString(product.ACTIVATION_TOKEN, pid);
    const reductions = parseTokenString(product.REDUCTION_TOKEN, pid);
    const shipments = parseTokenString(product.SHIPMENT_TOKEN, pid);

    if (activations.length) map.activation[pid] = activations;
    if (reductions.length) map.reduction[pid] = reductions;
    if (shipments.length) map.shipment[pid] = shipments;
  });
...
useEffect(() => {
  const tokenMap = buildTokenMap(props.selectedProduct || []);
  console.log("🔥 Parsed Token Map:", tokenMap);
}, [props.products]);
