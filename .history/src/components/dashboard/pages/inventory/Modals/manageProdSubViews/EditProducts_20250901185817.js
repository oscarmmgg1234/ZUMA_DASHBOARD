...
+                  {selectedProduct?.NAME} — {route.charAt(0).toUpperCase() + route.slice(1)} Graph
+                </div>
+                <div className="flex items-center gap-2">
+                  <span className="text-cyan-300/80 text-sm hidden md:inline">
+                    Press <kbd className="px-1.5 py-0.5 bg-slate-700 rounded">Esc</kbd> to exit
+                  </span>
+                  <button
+                    onClick={() => setShowGraphFullscreen(false)}
...
+                  }
+                  setSelectedProduct={setSelectedProduct}
+                  containerHeight="calc(100vh - 160px)"+                  containerWidth="100%"
             />
            </div>
            </div>
         </div>
