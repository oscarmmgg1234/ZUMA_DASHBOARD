// NodeRenderer.jsx — Pure client-side precheck + arrow-key centering (no node moves)
// - Keeps original save + token logic intact
// - Adds keyboard navigation: Arrow keys pick nearest node in that direction and center it
// - Click still behaves the same, plus we center the clicked node
...
/* -------------------- Keyboard centering helpers (added) -------------------- */
const DIRS = {
  ArrowLeft:  { vx: -1, vy:  0 },
...
function pickNeighbor(currentId, key, nodes) {
  const dir = DIRS[key];
  if (!dir) return null;
  const cur = nodes.find((n) => n.id === currentId) || nodes[0];
  if (!cur) return null;
  ...
  for (const n of nodes) {
    ...
    const score = (1 - dot) * 1000 + len; // prefer alignment, then distance
    if (score < bestScore) {
      bestScore = score;
      bestId = n.id;
    }
...
function FlowComponentInner({ props }) {
  ...
  // Center helper with retry until measured
  ...
  const onNodeClick = useCallback((_, node) => {
    ...
    // keep original behavior, plus center clicked node
    if (node?.id) {
  ...
  const onKeyDown = useCallback(
    (e) => {
      ...
      // NEW: Arrow-key navigation -> pick neighbor and center it
      if (e.key in DIRS) {
        e.preventDefault();
