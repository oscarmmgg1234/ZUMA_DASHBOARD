...
const runtimeTestHandler = async (productID) => {
  const win = window.open("", "_blank"); // no noopener/noreferrer
  if (!win) return alert("Popup blocked.");

  // optional placeholder
  win.document.write("<h2>Generating report…</h2>");
  win.document.close();

  const html = await http.runtimeTest({ productID }); // returns string
