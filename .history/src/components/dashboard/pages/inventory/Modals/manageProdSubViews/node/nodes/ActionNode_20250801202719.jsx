...
useEffect(() => {
  if (data.token) {
    if (selectedClass !== data.token.type) {
      setSelectedClass(data.token.type || '');
    }
    if (selectedFunctionId !== data.token.func) {
      setSelectedFunctionId(data.token.func || '');
    }
    if (param1 !== data.token.param1) {
      setParam1(data.token.param1 || '');
    }
    if (param2 !== data.token.param2) {
      setParam2(data.token.param2 || '');
    }
    if (param3 !== data.token.param3) {
  ...
  useEffect(() => {
  console.log("Received token in node:", data.token);
}, [data.token]);

  // 🔁 Update function list when class changes
  useEffect(() => {
    ...
    >
      ...
      <select
        ...
        onChange={(e) => {
          ...
          setSelectedFunctionId(''); // Reset function if class changes
        }}
      >
