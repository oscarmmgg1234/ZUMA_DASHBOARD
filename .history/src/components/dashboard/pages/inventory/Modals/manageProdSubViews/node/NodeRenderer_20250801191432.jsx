...
function buildLayout(tree, props) {
  const nodes = [];
  const edges = [];
  ...
  type: 'actionNode',
  data: {
    route: parentProduct.route, // e.g., "activation"
    registryMap, // from prepareRegistry
...
console.log(registryMap.get('AC'));
useEffect(() => {
  ...
  useEffect(() => {
    const { nodes, edges } = buildLayout(tree, props);
    setNodes(nodes);
    setEdges(edges);
  }, [tree]);
