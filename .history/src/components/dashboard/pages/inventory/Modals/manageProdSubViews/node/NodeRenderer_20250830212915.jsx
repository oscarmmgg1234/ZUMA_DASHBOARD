...
function FlowComponentInner({ props }) {
  ...
  useEffect(() => {
    ...
    // (Optional) If you want to visually reflect cleaned strings in the local product object only,
    // uncomment below. This DOES NOT touch the DB; only your React state.
    // props.setSelectedProduct?.((prev) =>
    //   prev && prev.PRODUCT_ID === p.PRODUCT_ID
    //     ? {
    //         ...prev,
  ...
  const handleCommit = useCallback(async () => {
    ...
    setCleanedTokens((prev) =>
      prev && prev.productID === props.selectedProduct.PRODUCT_ID
        ? { ...prev, [props.route]: norm(updatedToken) }
        : prev
    );
    ...
    props.setSelectedProduct?.((prev) =>
      prev && prev.PRODUCT_ID === props.selectedProduct.PRODUCT_ID
        ? { ...prev, [columnByRoute[props.route]]: norm(updatedToken) }
        : prev
    );
