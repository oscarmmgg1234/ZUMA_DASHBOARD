// Cleaned version with tables removed, form preserved + POOL LINK EDITOR
...
export default function EditProduct(props) {
  ...
  const linkToExistingPool = async () => {
    ...
    setPoolLoading(true);
    try {
      // normalizeRatio used to set/update ratio for the product in the pool link
      ...
      if (res?.linkedProduct) {
        // update local product & list to reflect the new link
        const updated = {
          ...selectedProduct,
        ...
        if (!virtualPools.length) await refreshPools();
      } else {
  ...
  const removeLink = async () => {
    if (!selectedProduct) return;
    const currentPoolId = getCurrentPoolId(selectedProduct);
    ...
    <div className="relative h-[62vh]">
      ...
      {selectedProduct && (
        <div className="mt-4">
          ...
          {/* ======== NEW: POOL LINK SECTION ======== */}
          <div className="mb-6 p-4 rounded border border-gray-300 bg-gray-50">
            ...
            <div className="mb-3 text-sm">
              <span className="text-black">
                ...
                <code className="text-blue-700">
                  {getCurrentPoolId(selectedProduct) ?? "none"}
                </code>
              </span>
              {Number.isFinite(
                selectedProduct?.poolRef?.ratio ?? selectedProduct?.POOL_RATIO
              ) && (
            ...
            {/* Ratio editor */}
            <div className="mb-3">
              <label className="block mb-1 text-black">Normalize Ratio</label>
              <input
                type="number"
                step="0.0001"
            ...
            <div className="flex flex-col md:flex-row gap-3">
              <div className="flex-1">
                ...
                <button
                  ...
                  disabled={!selectedPoolId || poolLoading}
                  className="mt-2 w-full md:w-auto px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded disabled:opacity-60"
                >
                  Link to Selected Pool
                </button>
              </div>

              <div className="flex-1">
                <label className="block mb-1 text-black">Create New Pool</label>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={!poolName.trim() || poolLoading}
            ...
            <div className="mt-3">
              <button
                onClick={removeLink}
                disabled={!getCurrentPoolId(selectedProduct) || poolLoading}
