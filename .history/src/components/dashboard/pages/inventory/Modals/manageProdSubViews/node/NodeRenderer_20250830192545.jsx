...
// --------- PRECHECK DEDUPE/CACHE (module-scope) ----------
const _precheckCache = new Map(); // key: `${productId}::${token}`
const _cacheKey = (productId, token) => `${productId}::${token || ""}`;

async function _safePrecheck(productId, tokenStr) {
  const token = (tokenStr || "").trim();
  if (!token) return { cleanedToken: "" }; // no-op if empty token

  const key = _cacheKey(productId, token);
  if (_precheckCache.has(key)) return _precheckCache.get(key);

  const p = http
    ._tokenPreCheck({ token, productID: productId })
    .catch((e) => {
      _precheckCache.delete(key); // don't cache failures
      throw e;
    });

  _precheckCache.set(key, p);
  return p;
}
// --------------------------------------------------------
...
const parseTokensFromProduct = async (product) => {
  ...
  // run prechecks in parallel, dedup + skip empty
  const [actRes, redRes, shipRes] = await Promise.all([
    _safePrecheck(product.PRODUCT_ID, product.ACTIVATION_TOKEN),
    _safePrecheck(product.PRODUCT_ID, product.REDUCTION_TOKEN),
    _safePrecheck(product.PRODUCT_ID, product.SHIPMENT_TOKEN),
  ]);
...
function FlowComponentInner({ props }) {
  ...
  // NEW: Warm precheck on product select (runs on load + whenever product changes)
  useEffect(() => {
    _precheckCache.clear(); // ensure fresh calls per selected product
    const p = props.selectedProduct;
    if (!p) return;

    // fire-and-forget warm-up; parseTokensFromProduct will also call these
    Promise.all([
      _safePrecheck(p.PRODUCT_ID, p.ACTIVATION_TOKEN),
      _safePrecheck(p.PRODUCT_ID, p.REDUCTION_TOKEN),
      _safePrecheck(p.PRODUCT_ID, p.SHIPMENT_TOKEN),
    ]).catch(() => {});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Only reuse refTree if it belongs to current product
      const refTreeMatches =
        Array.isArray(props.refTree) &&
        props.refTree?.[0]?.selectedProductId === props.selectedProduct.PRODUCT_ID;

      if (props.refTree && refTreeMatches) {
        setTree(props.refTree);
        setTreeSnapshot(JSON.parse(JSON.stringify(props.refTree)));
        return;
      } else if (props.refTree && !refTreeMatches) {
        // optional: clear refTree when switching product
        props.setRefTree?.(null);
      }
  ...
  }, [props.selectedProduct, props.route, props.products, props.refTree, props.setRefTree]);
