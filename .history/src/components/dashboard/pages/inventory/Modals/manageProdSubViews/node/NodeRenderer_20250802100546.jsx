...
const buildChangeValidator = (selectedProduct, allProducts, firstStage) => {
  ...
  for (const route of routes) {
    ...
    const productMap = firstStage.get(route);
    if (!productMap) {
      validatorMap.set(route, routeMap); // Empty map
      continue;
    }
    ...
    for (const [productId, tokens] of productMap.entries()) {
      ...
      const productTree = {
        id: `prod-${productId}`,
        name: product?.NAME || `Product ${productId.slice(0, 4)}`,
        type: 'product',
        selectedProductId: productId,
        children
      };

      routeMap.set(productId, productTree);
    }

    validatorMap.set(route, routeMap);
  }
...
function commitChanges(treeBeforeSnap, treeAfter){
...
useEffect(() => {
  ...
  const treeSnapshot = buildChangeValidator(props.selectedProduct, props.products, firstStage)
  
  setTree(newTree);
  setTreeSnapshot(treeSnapshot);
