...
const handleCommit = (tree, treeSnapshot, route, set) => {
  const isValid = commitChangesIsValid(tree, treeSnapshot);
  const updatedToken = treeToTokenEncoder(tree, route);
  console.log("updated token", updatedToken);

  if (!isValid) {
    setNotification("Token updated for this component.");
    return true; // Signal that changes exist
  } else {
    setNotification("No changes to commit.");
