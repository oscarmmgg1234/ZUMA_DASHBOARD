...
for (const productNode of tree) {
  if (!productNode.children || productNode.children.length === 0) continue;

  console.log(`🧩 Inspecting product node:`, productNode.id);
  console.log(`↪️ Children:`, productNode.children);

  for (const child of productNode.children) {
    console.log(`🔎 Child fields:`, Object.keys(child));
    console.log(`🧪 class: ${child.class}, func: ${child.func}, funcID: ${child.functionID}`);
  }
}

  // 🔍 Traverse tree to extract postops
  let postops = [];
  ...
  for (const productNode of tree) {
    ...
    const match = productNode.children.find(child =>
      ['29wp', '2a1k'].includes(child.name)
    );

    if (match) {
      postops.push({
        productID: productNode.id,
        ratio: match.param1 !== "" ? parseFloat(match.param1) : null
      });
    }
  }

  // 🧠 Encode token based on updated tree + route
