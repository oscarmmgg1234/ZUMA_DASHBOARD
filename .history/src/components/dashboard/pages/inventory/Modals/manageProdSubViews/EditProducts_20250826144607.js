...
// Replace your runtimeTestHandler with this:
const runtimeTestHandler = async (productID) => {
  try {
    // 1) Call your API
    const res = await http.runtimeTest({ productID });

    // 2) Normalize to an HTML string (covers: string, {html}, {data: {html}})
    const html =
      typeof res === "string"
        ? res
        : res.data ?? res.virtualPoolProductAdd ?? "<!doctype html><title>Report</title><h1>No HTML returned</h1>";

    // 3) Open a new tab and render the HTML string
    const win = window.open("", "_blank", "noopener,noreferrer");
    if (!win) {
      alert("Popup blocked. Please allow popups for this site.");
      return;
    }
    // fully replace the document with your report
    win.document.open();
    ...
    alert("Runtime test failed. See console for details.");
  }
};
