...
const handleCommit = (tree, treeSnapshot, route, setNotification) => {
  ...
  let postops = []
  if(isValid){
    //so we need to dig in the tree traverese the tree and check if the function id == 29wp or 2a1k
    //if this is the case then we must create {productID: the parentID, ratio: this will be the first param of the child of not then null but still pass}
    //we only need one for evey product that contains that function id
    //so lets use tree to get this done then populate postops[{}]
  ...
  if (!isValid) {
  setNotification({ message: "Changes being saved.", type: 'success' });
  return true;
} else {
...
const onKeyDown = useCallback((e) => {
  ...
  <button
    onClick={() => {
      const isValid = handleCommit(tree, treeSnapshot, props.route, setNotification);
      if (!isValid) {
        setTreeSnapshot(JSON.parse(JSON.stringify(tree)));
      }
    }}
