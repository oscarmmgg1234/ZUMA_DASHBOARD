...
/** Parse space-separated token string into token objects (excluding preops/postops/virtualops) */
...
/** Build {activation,reduction,shipment} arrays from either cleaned tokens (preferred) or raw product tokens */
const parseTokensFromProduct = (product, cleanedTokens /* optional */) => {
  const act = (cleanedTokens?.activation ?? product.ACTIVATION_TOKEN ?? "").trim();
  const red = (cleanedTokens?.reduction  ?? product.REDUCTION_TOKEN  ?? "").trim();
  const ship = (cleanedTokens?.shipment ?? product.SHIPMENT_TOKEN   ?? "").trim();

  return {
    activation: parseTokenString(act,  product.PRODUCT_ID),
    reduction:  parseTokenString(red,  product.PRODUCT_ID),
    shipment:   parseTokenString(ship, product.PRODUCT_ID),
  };
};

/** Build first stage map from parsed tokens (no network calls here) */
const buildFirstStageMap = (product, parsed /* required here */) => {
...
/* --------------------------- PRECHECK ONLY ON SELECT --------------------------- */
/** One-time (per product select) batch precheck; returns cleaned token strings. */
async function precheckAllTokens(product) {
  ...
  const calls = [
    (async () => {
      ...
      if (!t) return "";
      const r = await http._tokenPreCheck({ token: t, productID: pid });
  ...
  const [activation, reduction, shipment] = await Promise.all(calls);
...
const handleCommit = async (
  ...
  cleanedTokens    // <-- ensure commit logic uses the same prechecked tokens
) => {
...
function FlowComponentInner({ props }) {
  ...
  // 1) Run ONE precheck batch whenever selected product changes
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const p = props.selectedProduct;
      if (!p) { setCleanedTokens(null); return; }
      try {
        const res = await precheckAllTokens(p);
        if (cancelled) return;
        setCleanedTokens(res); // {productID, activation, reduction, shipment}
      } catch (e) {
        if (cancelled) return;
        // Fail gracefully by falling back to raw tokens
        setCleanedTokens({
  ...
  // 2) Build the visual tree from the cleaned tokens (no extra prechecks)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      ...
      const firstStage = buildFirstStageMap(props.selectedProduct, parsed);
      if (cancelled) return;
  ...
  // 3) Rebuild RF nodes/edges whenever the visual tree changes
  useEffect(() => {
      ...
      <button
        onClick={async () => {
          const isSame = await handleCommit(
            ...
            props.route,
            setNotification,
            props.selectedProduct,  // full product
            cleanedTokens           // use the prechecked tokens from load
          );
