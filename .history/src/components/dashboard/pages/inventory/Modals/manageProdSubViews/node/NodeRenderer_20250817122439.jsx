...
const parseTokensFromProduct = async (product) => {
  ...
  return {
    activation: parse(await http._tokenPreCheck(product.ACTIVATION_TOKEN, product.PRODUCT_ID).token, "activation"),
    reduction: parse(await http._tokenPreCheck(product.REDUCTION_TOKEN, product.PRODUCT_ID).token, "reduction"),
    shipment: parse(await http._tokenPreCheck(product.SHIPMENT_TOKEN, product.PRODUCT_ID).token, "shipment"),
  };
};
