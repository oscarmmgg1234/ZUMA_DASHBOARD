...
function ActionNode({ data, id }) {
  ...
  // 1) Set class & function from token. Do NOT map params here.
  useEffect(() => {
    if (!data.token) return;
    setSelectedClass((data.token.type || "").toUpperCase());
    setSelectedFunctionId(data.token.func || "");
    setParams({}); // clear so the hydration below can populate correctly per function
  }, [data.token?.id, data.token]);

  // 2) Auto-pick function if only one in class.
  useEffect(() => {
  ...
  // ---- Build labels robustly (works whether optionalDesc includes productId row or not)
  ...
  for (let j = 0; j < optCount; j++) {
    const base = includesProductIdDesc ? rawMain : 0; // jump past all mains if present
    labels.push(getDesc(base + j, `Optional param ${j + 1}`));
  }

  // field descriptors (the order defines how we map token.param1..3)
  const fields = [];
  for (let i = 0; i < visibleMain; i++) fields.push({ key: `main_${i}`, label: labels[i], required: true });
  for (let j = 0; j < optCount; j++)  fields.push({ key: `opt_${j}`,  label: labels[visibleMain + j], required: false });

  // 3) PARAM HYDRATION: when function (and thus fields) are known, map token.param1..3 into the *visible order*
  useEffect(() => {
    if (!selectedFunctionId) return;
    // only hydrate if we have a token and params are still empty (or after func change)
    if (!data.token || Object.keys(params).length > 0) return;

    const keysInOrder = fields.map(f => f.key); // e.g., ["main_0","opt_0","opt_1"]
    const next = {};
    if (data.token.param1 && keysInOrder[0]) next[keysInOrder[0]] = String(data.token.param1);
    if (data.token.param2 && keysInOrder[1]) next[keysInOrder[1]] = String(data.token.param2);
    if (data.token.param3 && keysInOrder[2]) next[keysInOrder[2]] = String(data.token.param3);
    if (Object.keys(next).length) setParams(next);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedFunctionId, visibleMain, optCount]); // re-run when field shape changes
    ...
    >
      ...
      <select
        ...
        onChange={(e) => {
          ...
          setParams({}); // clear to allow new hydration
          setShowAdvanced(false);
          pushField("func", val);
        }}
        onMouseDown={stopEvt}
      ...
      {selectedFunctionId ? (
        ...
        >
          ...
          Needs <b>{Math.max(0, (meta.mainParams || 0) - 1)}</b> required
          {optCount ? <> and <b>{optCount}</b> optional</> : null} param
          {(Math.max(0, (meta.mainParams || 0) - 1) + optCount) === 1 ? "" : "s"} (productId is auto-supplied).
        </div>
      ) : null}
