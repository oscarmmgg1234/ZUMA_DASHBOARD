 ...
 * - Removes extra-params modal; sends only { PRODUCT_ID, QUANTITY, ACTION }
 * - ACTION ∈ {"STORED_STOCK" | "ACTIVE_STOCK"} (still required)
 * - Optimistic UI, per-row saving pulse, success highlight
...
export default function OverrideStock(props) {
  ...
  const [rowSavingId, setRowSavingId] = useState(null); // row pulse while saving
  const [flashCellKey, setFlashCellKey] = useState(""); // cell success flash
  ...
  const handleEditClick = (product, field) => {
    ...
    setQuantity(String(Number(safe).toFixed(2)));
  };
  ...
  const commitChange = async () => {
    ...
    // Minimal packet + ACTION (required)
    const payload = {
    ...
    try {
      ...
      // success flash on the edited cell
  ...
  const CellFlash = ({ cellKey, children }) => {
    // Tailwind-based transient flash
    return (
      <span
        key={cellKey}
        className="inline-block rounded px-1 transition-colors duration-700 bg-emerald-100"
        onAnimationEnd={() => {}}
      >
        {children}
      </span>
          ...
          {isEditingStored ? (
            ...
            cellKeyStored ? (
              <CellFlash cellKey={cellKeyStored}>
                {inv.STORED_STOCK.toFixed(2)}
              </CellFlash>
            ) : (
              <span className="text-black">{inv.STORED_STOCK.toFixed(2)}</span>
            )
          ) : (
            "N/A"
          )}
          ...
          {isEditingActive ? (
            ...
            cellKeyActive ? (
              <CellFlash cellKey={cellKeyActive}>
                {inv.ACTIVE_STOCK.toFixed(2)}
              </CellFlash>
            ) : (
              <span className="text-black">{inv.ACTIVE_STOCK.toFixed(2)}</span>
            )
          ) : (
            "N/A"
          )}
